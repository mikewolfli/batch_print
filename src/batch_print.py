    ...
    def handle_starttag(self, tag, attrs):
        if tag=='span':
            for name,value in attrs:
                if name=='id' and value==self.idvalue:
                    self.a_t=1
    
    def handle_endtag(self, tag):
        if tag=='span' and self.a_t==1:
            self.a_t=0
...
class Application(Frame):
    ...
    def log_print(self,wbs):
        cur_unit=self.conn.cursor()
        cur_unit.execute("INSERT INTO l_print_log(print_content, printed_by, printed_at, printer) VALUES \
                        ('"+wbs+"','"+self.user_id+"','"+time.strftime('%Y-%m-%d %H:%M:%S', time.localtime() )+"','SLO-P3106(U)');")
        self.conn.commit()
    ...
    def parse_xhtml(self, result):
        ...
        for item in dic_name:            
            p_parser=myparser(id_pre+item)
            p_parser.feed(result)
            if p_parser.a_t==2:
                return None
            
            data[item]=p_parser.get_value()
    ...
    def print_wbs(self,wbs):
        ...
        #self.logger.warn("启动小标签打印机，同时打开模板文件")
        btformt = btEngine.Documents.Open(wbs_file_path)
        
        #self.logger.warn("小标签打印开始...")
        for wbs_item in wbs:
            btformt.SubStrings.SetSubString('project_id',wbs_item[0:10])
            btformt.SubStrings.SetSubString('unit_id', wbs_item[11:])
            result=btformt.Print()
            
            if result:
                self.logger.warn(wbs_item+":小标签打印成功")
            else:
                self.logger.error(wbs_item+":小标签打印失败")
       
        #self.logger.warn("小标签打印任务结束")
    ...
    def print_info(self,wbs): 
        ...
        #self.logger.warn("启动信息表标签打印机，同时打开模板文件")
        btformt = btEngine.Documents.Open(wbs_file_path)
        
        force_print = self.force_print_value.get()
        
        #self.logger.warn("信息表标签打印开始...")
        ...
        for data_line in data_pool:
            ...
            for dic2 in dic_name2:
                if not flag and (dic2=='SaftyGearProductionNoCw' or dic2=='GovernorProductionNoCw'):
                    value=none_str
            ...
            if result:
                self.logger.warn(data_line['WBSNo']+":信息表标签打印成功")
                self.log_print(data_line['WBSNo'])               
            else:
                self.logger.error(data_line['WBSNo']+":信息表标签打印失败")
      
        #self.logger.warn("信息表标签打印任务结束")            
